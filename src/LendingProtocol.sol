...
contract LendingProtocol {
    ...
    constructor(
        ...
        pair = address(_pair);
        if (
            !((_pair.token0() == _ctf && _pair.token1() == _usd) || (_pair.token0() == _ctf && _pair.token1() == _usd))
        ) {
    ...
    function _requireOverCollateralized(Position storage collateral, Position storage debt) internal view {
        ...
        if (collateralValue < debtValue) revert UnderCollateralized(collateralValue, debtValue);
    }
